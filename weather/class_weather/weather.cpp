...
void WEATHER::sendNetworkRequest(QString procince, QString city)
{
    ...
    url.append("&key=57aec39a46b847478ec2a938c6873e1d");
...
QString WEATHER::getUpdateTime(QJsonObject obj)
{
    ...
    cityInfoObj=obj.value("now").toObject();
    time=cityInfoObj.value("obsTime").toString().mid(11,5);
    updateTime=time;
...
QString WEATHER::getDataAndTime(QJsonObject obj)
{
    ...
    date = obj.value("updateTime").toString().mid(0, 10);
    time = obj.value("updateTime").toString().mid(11, 5);

    date = date.replace(4, 1, tr("-"));
    date = date.replace(7, 1, tr("-"));
    date.append(tr("-"));
...
QString WEATHER::getRealTimeTemp(QJsonObject obj)
{
    ...
    dataObj = obj.value("now").toObject();

    realTimeTemp = dataObj.value("temp").toString();
...
QString WEATHER::getRealTimeAirQuality(QJsonObject obj)
{
    ...
    airQuality = dataObj.value("vis").toString();
...
*/
    dataObj=obj.value("now").toObject();
    forecastTemp="体感温度："+dataObj.value("feelsLike").toString()+"℃";
...
QString WEATHER::getForecastWeatherType(QJsonObject obj, quint8 index)
{
    ...
    dataObj=obj.value("now").toObject();
    forecastWeatherType=dataObj.value("text").toString();
...
QString WEATHER::getForecastDataAndWeek(QJsonObject obj, quint8 index)
{
    ...
    forecastDate = obj.value("updateTime").toString().mid(0, 10);
    forecastTime = obj.value("updateTime").toString().mid(11, 5);

    forecastDate = forecastDate.replace(4, 1, tr("-"));
    forecastDate = forecastDate.replace(7, 1, tr("-"));
    //forecastDate.append(tr("-"));

    forecastDateAndWeek = forecastDate + " " + forecastTime;
